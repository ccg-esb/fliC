...
%{
figure(4)
clf('reset'); set(gca, 'fontsize', 12); set(gcf, 'DefaultLineLineWidth',2); set(gcf,'color','white');
    
% first plot
subplot(3,1,1)
plot(t*1e6,x,'k')
xlabel('Time (seconds)')
axis([0 t(end)*1e6 0 1.05])
ylabel('Mean GFP')
title('Original GFP signal')

%second plot
subplot(3,1,2)
yplot=abs(y(1:1+Nfft/2));
yplot=yplot/max(yplot);
semilogy(f*1e-6, yplot, 'k'); hold on
semilogy(f(ind1)*1e-6, yplot(ind1), 'or', 'LineWidth', 1);
xlabel('Frequency (MHz)')
title('Amplitudes')
ylabel('Power')
legend('Spectrum', 'Selected frequencies')
legend boxoff

% third plot
subplot(3,1,3)
plot(t*1e6,X.*max(x), 'k')
xlabel('Time (seconds)')
axis([0 t(end)*1e6 0 1.05])
ylabel('Mean GFP')
